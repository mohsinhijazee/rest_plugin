...
hash = account.attributes.symbolize_keys
to_be_skipped.each {|attr| hash.delete attr }

hash[:url]                = url(account_url(account))
hash[:account_type_url]   =  url(account_type_url(:id => account.account_type_id))
hash[:databases_url]       = url(account_databases_url(:account_id => account.id))
